...
export const main = async () => {
    const filePaths = fs.readdirSync('./out', {recursive: true, withFileTypes: true})
    const envFile = fs.readFileSync('.env.production', {encoding: 'utf-8'})
    const publicPathEnv = envFile.split('\n').find(env => env.includes('NEXT_PUBLIC_ROUTE_HOME'))
    const publicPath = publicPathEnv?.split('=/')[1]
    const zipBundleEnv = envFile.split('\n').find(env => env.includes('NETSUITE_ZIP_BUNDLE'))
    ...
    filePaths.forEach((path) => {
        if(path.isFile()){
            ...
            if(fileNeedUpgrade){
                const file =  fs.readFileSync(`${path.parentPath}/${path.name}`, {encoding: 'utf-8'})

                const newFile = file.replaceAll(nextPublicPath, netsuitePublicPath)

                fs.writeFileSync(`${path.parentPath}/${path.name}`, newFile, {encoding: 'utf-8'})
            }
        }
    })

import { useWindowResize } from "@workspace/ui/hooks"
import React, { createContext, Dispatch, PropsWithChildren, RefObject, SetStateAction, useContext, useEffect, useMemo, useRef, useState } from "react"
...
export const VirtualList: React.FC<VirtualListProps> = ({
    ...
    const itemsCountInterval = useRef<ReturnType<typeof setInterval>>()
    ...
    const content: VirtualiListContent = showList 
        ? 'list' 
        : 'list-placeholder'
...
export const VirtualListProvider: React.FC<VirtualListProviderProps> = ({
    children,
    loading
}) => {
    return (
        <virtualListContext.Provider value={{loading}}>
            {children}
...
export const useVirtualSize = (
    ...
    const { itemsSize, setContainer, setItemsToMeasure, setExtraSpace, setFilledSpace, setRedoOnResize, loading } = context
